...
import { X, User, Calendar, Tag, MessageSquareText, ChevronDown, Route } from "lucide-react";
...
export const TIPO_META = {
  averia: { label: "Avería", color: "bg-[#FFE8CC] text-[#8A5A00]" },
  incidente: { label: "Incidente", color: "bg-[#FDE8E8] text-[#8A0000]" },
  reclamo: { label: "Reclamo", color: "bg-[#E3EAFC] text-[#2C4A9E]" },
  sugerencia: { label: "Sugerencia", color: "bg-[#D8F3DC] text-[#1B4332]" },
};

const ESTADOS = Object.entries(ESTADO_META).map(([value, meta]) => ({ value, ...meta }));

export default function ReporteConductorModal({ open, reporte, onClose, onResponder }) {
  const initialMode = useMemo(() => (reporte?.respuesta_admin ? "responder" : "ver"), [reporte]);
  ...
  const tipoMeta =
    TIPO_META[reporte.tipo_reporte] ?? ({ label: reporte.tipo_reporte, color: "bg-neutral-100 text-neutral-600" });
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-neutral-50 border border-neutral-100 px-3.5 py-3 flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#14201B]">{reporte.conductor}</p>
                <p className="text-xs text-neutral-400 mt-0.5">DNI: {reporte.dni}</p>
              </div>
            </div>
            <div className="rounded-lg bg-neutral-50 border border-neutral-100 px-3.5 py-3 flex items-start gap-2.5">
              <Route className="h-4 w-4 text-[#40916C] mt-0.5 shrink-0" />
              <div>
                <p className="text-[10px] text-neutral-400 uppercase tracking-wide">Ruta / Unidad</p>
                <p className="text-sm font-medium text-[#14201B]">{reporte.ruta}</p>
                <p className="text-xs text-neutral-400 mt-0.5">Placa: {reporte.placa}</p>
              </div>
            </div>
            <div className="rounded-lg bg-neutral-50 border border-neutral-100 px-3.5 py-3 flex items-start gap-2.5">
          ...
          <div>
            ...
            {mode === "ver" ? (
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-neutral-600 mb-1.5">Actualizar estado</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-neutral-600 mb-1.5">Mensaje al conductor</label>
                  <textarea
                    value={respuesta}
                    onChange={(e) => setRespuesta(e.target.value)}
                    rows={4}
                    placeholder="Ej. Hemos registrado el incidente y coordinaremos la solución en el próximo turno..."
